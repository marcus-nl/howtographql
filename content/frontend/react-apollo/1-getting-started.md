...
`App.js` is a component, so move it into `components`. `App.css` and `index.css` contain styles, so move it into `styles`. You also need to change the references to these files in `index.js` accordingly:
